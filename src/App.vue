    ...
    <div class="container">
        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            <div class="relative mt-1 rounded-md shadow-md">
              <input type="text" name="wallet" id="wallet" @keyup.enter="addTicker" v-model="tickerInputValue"
            ...
            <div class="text-sm text-red-600" v-if="isShowTooltipForSameTicker">Такой тикер уже добавлен</div>
          </div>
        </div>
      ...
      <template v-if="tickers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div class="cursor-pointer overflow-hidden rounded-lg border-solid border-purple-800 bg-white shadow"
            v-for="ticker in tickers" :key="ticker.label" @click="sel = ticker" :class="{
              'border-4'
            }">
            ...
            <button @click="onDelete(ticker.label)"
...
<script>
const api_key = "f4829e03fd35e85421baf7d5b747d5ccaf6ba31428a825af81659e9f02fb8777"
export default {
  name: 'App',
  ...
  methods: {
    addTicker() {
      ...
      setInterval(async () => {
        const f = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${newTicker.label}&tsyms=USD&api_key=${api_key}`)
